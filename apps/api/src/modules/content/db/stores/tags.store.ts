...
export class TagsStore {
    ...
    async fetchTagsTrees(kind: TagKind): Promise<TagsTree[]> {
        ...
        const results: TagsTree[] = await this.tags
            .aggregate<TagsTree>([{ $match: { kind: kind, parent: null } }])
            .sort({ name: 1 })
            .graphLookup({
                from: 'tags',
                startWith: '$_id',
    ...
    private async populateImmediateChildren(id: string): Promise<TagsTree> {
        ...
        // put it together ourselves anyway.
        const results: TagsTree[] = await this.tags
            .aggregate<TagsTree>([{ $match: { _id: id } }])
            .graphLookup({
                from: 'tags',
                startWith: '$_id',
                connectFromField: '_id',
    ...
    async createTag(form: TagsForm, tagKind: TagKind): Promise<TagsDocument> {
        if (form.parent) {
            ...
            if (parentTag.kind !== tagKind) {
                throw new BadRequestException(
                    "The parent tag's kind must match the new tag's kind.",
                );
            }
        }
        ...
        if (form.parent) {
            const tagWithSameName = await this.findChildTagByNameAndParent(
                newTag.name,
                form.parent,
            );
            if (tagWithSameName) {
                throw new BadRequestException(
                    'There is already a child tag of this parent with this name.',
                );
            }
        }
    ...
    async updateTag(tagId: string, form: TagsForm): Promise<void> {
        ...
        if (form.parent) {
            ...
            if (tagWithSameName && tagId !== tagWithSameName._id) {
                throw new BadRequestException(
                    'There is already another child tag of this parent with this name.',
                );
            }
        }
        ...
        // Don't check for duplicates with children
            ...
            if (tagWithSameName && tagId !== tagWithSameName._id) {
                throw new BadRequestException(
                    'There is already another parent tag with this name.',
                );
            }
        }
