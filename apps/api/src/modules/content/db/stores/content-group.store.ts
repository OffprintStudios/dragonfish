...
export class ContentGroupStore {
    ...
    async fetchFirstThreePublished(
        ...
        const filteredWorksQuery = await ContentGroupStore.determineContentFilter(
            worksQuery,
            filter,
        );
        ...
        const filteredBlogsQuery = await ContentGroupStore.determineContentFilter(
            blogsQuery,
            filter,
        );
        ...
        if (genres && genres.length > 0) {
            switch (genreSearchMatch) {
                ...
                case SearchMatch.NoOthers:
                    paginateQuery['meta.genres'] = {
                        $not: { $elemMatch: { $nin: genres } },
                        $exists: true,
                    };
            ...
            switch (tagSearchMatch) {
                ...
                case SearchMatch.NoOthers:
                    if (includeChildTags) {
                        ...
                        paginateQuery['tags'] = {
                            $not: { $elemMatch: { $nin: allTags } },
                            $exists: true,
                        };
                        paginateQuery['tags.0'] = { $exists: true };
                    } else {
                        paginateQuery['tags'] = {
                            $not: { $elemMatch: { $nin: tagIds } },
                            $exists: true,
                        };
                        paginateQuery['tags.0'] = { $exists: true };
                    }
     ...
     * @param contentId
     */
    private async addOrFetchRatingsDoc(
        user: JwtPayload,
        contentId: string,
