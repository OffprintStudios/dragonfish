...
export class ImagesService {
  ...
  private validateImageFile(file: any): void {
    ...
    if (
      ...
      file.mimetype !== MimeTypes.jpg &&
      file.mimetype !== MimeTypes.png
    ) {
      throw new BadRequestException(
        `Upload must be either a JPEG or PNG image!`,
      );
    }
  }

  private getExtension(file: any): string {
    if (file.mimetype === MimeTypes.jpeg || file.mimetype === MimeTypes.jpg) {
      return 'jpg';
    } else if (file.mimetype === MimeTypes.png) {
      return 'png';
    }
  }
}
