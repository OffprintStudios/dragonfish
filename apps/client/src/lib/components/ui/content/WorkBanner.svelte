<script lang="ts">
    ...
    const rating = useQuery('contentRatings', () => fetchRatings(content._id), {
        enabled: !!$session.account && !!content && content.audit.published === PubStatus.Published,
        cacheTime: 1500,
    });

    const updateVote = useMutation(
        (ratingOption: RatingOption) => changeVote(content._id, ratingOption),
        {
            onSuccess: (data) => {
...
{#if $session && content}
    <div class="work-banner">
        ...
        >
            ...
            {:else if $session.currProfile && $session.currProfile._id === content.author._id}
            ...
            <div class="flex flex-col md:flex-row flex-1 md:items-center md:my-4">
                ...
                <div class="flex items-center justify-center md:justify-start">
                    {#if $session.account && $rating && $rating.data}
                        {#if $rating.data.rating === RatingOption.Liked}
                            <Button
                                ...
                                loadingText={content.stats.likes}
                            >
                            ...
                            <Button
                                ...
                                title="Dislike"
                                loading={$updateVote.isLoading}
                                loadingText={content.stats.dislikes}
                            >
