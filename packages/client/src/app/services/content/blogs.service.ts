...
export class BlogsService {
    ...
    public createBlog(info: BlogForm, parentId?: Types.ObjectId) {
        if (parentId) {
            return this.http
                .put<models.Blog>(`${this.url}/create-blog?parentId=${parentId}`, info, {
                    observe: 'response',
            ...
            return this.http
                .put<models.Blog>(`${this.url}/create-blog`, info, { observe: 'response', withCredentials: true })
                .pipe(
                    map((res) => {
                        if (res.status === 201) {
                            this.alertsService.success('Blog successfully created.');
    ...
    public fetchUserBlogs(pageNum: number) {
        return this.http
            .get<PaginateResult<models.Blog>>(`${this.url}/fetch-user-blogs/${pageNum}`, {
                observe: 'response',
                withCredentials: true,
            })
            .pipe(
    ...
    public deleteBlog(blogId: string) {
        return this.http
            .patch(`${this.url}/delete-blog`, { blogId }, { observe: 'response', withCredentials: true })
            .pipe(
                map((res) => {
                    if (res.status === 200) {
    ...
    public changePublishStatus(contentId: string, pubChange: PubChange) {
        return this.http
            .patch(`${this.url}/set-publish-status/${contentId}`, pubChange, {
                observe: 'response',
                withCredentials: true,
            })
            .pipe(
    ...
    public editBlog(blogId: string, blogInfo: BlogForm) {
        return this.http
            .patch<BlogsContentModel>(`${this.url}/edit-blog/${blogId}`, blogInfo, {
                observe: 'response',
                withCredentials: true,
            })
            .pipe(
