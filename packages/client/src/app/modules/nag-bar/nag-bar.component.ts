...
export class NagBarComponent implements OnInit, OnDestroy {
    ...
    constructor(private injector: Injector, private nagBarService: NagBarService) {}

    ngOnInit(): void {
        this.contentChangedSubscription = this.nagBarService
            .onContentChanged()
            .subscribe((x) => this.onContentChanged(x));

        this.clearRequestedSubscription = this.nagBarService
            .onClearRequested()
