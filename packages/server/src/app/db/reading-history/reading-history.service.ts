...
export class ReadingHistoryService {
    ...
    async fetchUserHistory(user: JwtPayload, pageNum: number): Promise<PaginateResult<ReadingHistoryDocument>> {
        return await this.histModel.paginate({'owner': user.sub, 'visible': true}, {
            sort: {'viewedOn': -1},
            page: pageNum,
            limit: 15
        });
    ...
    async fetchUserSidenavHistory(user: JwtPayload): Promise<ReadingHistoryDocument[]> {
        return await this.histModel.find({'owner': user.sub, 'visible': true})
            .limit(7)
            .sort({'viewedOn': -1});
    }
    ...
    async changeVisibility(user: JwtPayload, histId: string): Promise<void> {
        return await this.histModel.updateOne({'_id': histId, 'owner': user.sub}, {
            'visible': false
        });
    }
}