...
export class ApprovalQueueStore {
     ...
     * @param pageNum
     */
    async fetchForMod(pseudId: string, pageNum: number): Promise<PaginateResult<ApprovalQueueDocument>> {
        return await this.approvalQueue.paginate(
            { claimedBy: pseudId },
            {
                sort: { createdAt: -1 },
    ...
    async claimWork(pseudId: string, docId: string): Promise<ApprovalQueueDocument> {
        ...
        if (thisEntry.claimedBy === null) {
            return this.approvalQueue.findOneAndUpdate({ _id: docId }, { claimedBy: pseudId }, { new: true });
        } else {
